...
import json from './presets/1.json';
...
class ImageMapEditor extends Component {
	...
	canvasHandlers = {
		...
		onChange: (selectedItem, changedValues, allValues) => {
			...
			if (changedKey === 'icon') {
				...
				if (styles[0] === 'brands') {
					this.canvasRef.handler.set('fontFamily', 'Spotify Font');
			...
			if (changedKey === 'fontWeight') {
				this.canvasRef.handler.set(changedKey, 'bold');
				return;
			}
	...
	handlers = {
		...
		onUsePreset: jsonData => {
			...
			// TODO: doesnt render font on initial JSON import, so running it twice for now works
			setTimeout(() => {
